...
export class SlitLevel {
  constructor(engine, arData) {
    ...
    this.screenHits = [];
    this.spawnTimer = 0;
    this.challengeStep = 1;  // 1: accumula 30 hit, 2: attiva detector
  ...
  _paintHit(y) {
    ...
    if (this.challengeStep === 1 && this.screenHits.length >= 30) {
      this.challengeStep = 2;
    }
  }

  getProgress() {
    if (this.challengeStep === 1) {
      return { current: Math.min(this.screenHits.length, 30), target: 30, label: 'Particelle accumulate' };
    }
    return { current: this.challengeDone ? 1 : 0, target: 1, label: 'Attiva il detector!' };
  }
  ...
  setDetector(on) {
    if (this.detectorOn !== on) {
      ...
      if (on && this.challengeStep === 2 && !this.challengeDone) {
        this.challengeDone = true;
        this.engine.containerEl.dispatchEvent(new CustomEvent('challenge-complete'));
      }
    }
  }
